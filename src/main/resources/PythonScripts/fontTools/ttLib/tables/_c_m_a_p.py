...
class table__c_m_a_p(DefaultTable.DefaultTable):
	...
	def decompile(self, data, ttFont):
		...
		for i in range(numSubTables):
			...
			if not length:
				print(
					"Error: cmap subtable is reported as having zero length: platformID %s, platEncID %s,  format %s offset %s. Skipping table." % (
...
class cmap_format_2(CmapSubtable):
	...
	def compile(self, ttFont):
			...
			if firstbyte != lastFirstByte:  # Need to update the current subhead, and start a new one.
				if lastFirstByte > -1:
					...
					assert (subHeader.entryCount == len(
						subHeader.glyphIndexArray)), "Error - subhead entry count does not match len of glyphID subrange."
				# init new subheader
				subHeader = SubHeader()
		...
		idRangeOffset = (len(
			subHeaderList) - 1) * 8 + 2  # offset to beginning of glyphIDArray from first subheader idRangeOffset.
		subheadRangeLen = len(
			subHeaderList) - 1  # skip last special empty-set subheader; we've already hardocodes its idRangeOffset to 2.
		...
		for subhead in subHeaderList[:-1]:
			length = length + len(
				subhead.glyphIndexArray) * 2  # We can't use subhead.entryCount, as some of the subhead may share subArrays.
		...
		for subhead in subHeaderList:
			dataList.append(struct.pack(subHeaderFormat, subhead.firstCode, subhead.entryCount, subhead.idDelta,
										subhead.idRangeOffset))
		for subhead in subHeaderList[:-1]:
			for gi in subhead.glyphIndexArray:
				dataList.append(struct.pack(">H", gi))
		data = bytesjoin(dataList)
		assert (len(data) == length), "Error: cmap format 2 is not same length as calculated! actual: " + str(
			len(data)) + " calc : " + str(length)
		return data
		...
		for i in range(len(startCode) - 1):  # don't do 0xffff!
			...
			if rangeOffset == 0:
				...
				for charCode in rangeCharCodes:
					...
					assert (
					index < lenGIArray), "In format 4 cmap, range (%d), the calculated index (%d) into the glyph index array  is not less than the length of the array (%d) !" % (
					i, index, lenGIArray)
...
class cmap_format_12_or_13(CmapSubtable):
	...
	def decompileHeader(self, data, ttFont):
		...
		assert len(data) == (16 + nGroups * 12) == (
		length), "corrupt cmap table format %d (data length: %d, header length: %d)" % (self.format, len(data), length)
		self.format = format
	...
	def compile(self, ttFont):
		if self.data:
			return struct.pack(">HHLLL", self.format, self.reserved, self.length, self.language,
							   self.nGroups) + self.data
		charCodes = list(self.cmap.keys())
		lenCharCodes = len(charCodes)
