    ...
    def compile(self, ttFont):
        ...
        for curStrike in self.strikes:
            ...
            for i in curStrike.indexSubTables:
                data = struct.pack(indexSubHeaderFormat, i.firstGlyphIndex, i.lastGlyphIndex,
...
class FixedSizeIndexSubTableMixin(object):
    ...
    def padBitmapData(self, data):
        ...
        assert len(
            data) <= self.imageSize, "Data in indexSubTable format %d must be less than the fixed size." % self.indexFormat
...
class eblc_index_sub_table_2(FixedSizeIndexSubTableMixin, EblcIndexSubTable):
    ...
    def compile(self, ttFont):
        ...
        assert glyphIds == list(
            range(self.firstGlyphIndex, self.lastGlyphIndex + 1)), "Format 2 ids must be consecutive."
